...
export const MainLayout: React.FC<MainLayoutProps> = ({ 
  ...
  const isConfigScreen = location.pathname === '/quiz/config' || location.pathname === '/idioms/config' || location.pathname === '/ows/config' || location.pathname === '/synonyms/config';
  ...
  return (
    <div className={cn(
        ...
        isAIFullScreen || isConfigScreen ? "h-[100dvh] w-screen overflow-hidden fixed inset-0" : "min-h-screen"
    )}>
      
      {/* --- Sticky Top Header --- */}
      {!isReviewMode && !isAIFullScreen && !isConfigScreen && (
      ...
      <nav className={cn(
        ...
        isReviewMode || isAIFullScreen || isConfigScreen ? "hidden" : "block"
      )}>
        {/* Glow Background Layer */}
        <div className="absolute inset-0 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl transition-colors duration-300 z-0"></div>
